...
encoded = open('records_encoded.json')
e_data = json.load(encoded)

records_encoded = e_data["records_encoded"]

decoded = open('records_decoded.json')
d_data = json.load(decoded)

records_decoded = d_data["records_decoded"]
...
def decode_measure():
    ...
    for i in range(k):
        #records_encoded
        #ll1 = records_encoded[i].split(";")[0]
        #ll2 = records_encoded[i].split(";")[1]
        print("Output of record "+str(i+1)+" : "+str(decode(records_encoded[i])))
    print("No of records processed: "+str(k))

    t2 = time.time()
    diff = (t2-t1)*1000
    #return diff
    print("The processing time taken: "+str(diff))
...
def encode_measure():
    ...
    for j in range(k):
        ...
        print("Output of record "+str(j+1)+" : "+str(encode(records_decoded[j])))
    print("No of records processed: "+str(k))

    t2 = time.time()
    diff2 = (t2-t1)*1000
    #return diff2
    print("The processing time taken: "+str(diff2))


def mismatch_measure():
    t1 = time.time()
    for i in range(k):
        #records_encoded
        #ll1 = records_encoded[i].split(";")[0]
        #ll2 = records_encoded[i].split(";")[1]
        print("Output of record "+str(i+1)+" : "+str(mismatch(records_encoded[i])))
    print("No of records processed: "+str(k))

    t2 = time.time()
    diff3 = (t2-t1)*1000
    #return diff3
    print("The processing time taken: "+str(diff3))

#print("The processing time taken: "+str(mismatch_measure()))
